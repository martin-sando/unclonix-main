...
def filter_points(points, img, name=""):
    ...
    for p in points:
        ...
        # Если точка достаточно лежит внутри TODO играть с этими параметрами
        if (p.point_coef > 0.999 and p.r >= 6 and p.strong >= 85) or (p.point_coef > 0.5 and p.r >= 8 and p.strong >= 136):  # Если точка достаточно лежит внутри
            new_points.append(p)
